...
def compute_difference_spectrogram(
    ...
    Handles shape mismatches by trimming to minimum length.
    """
    # Codec compression can slightly change length, trim to match
    min_len = min(spec_original.shape[1], spec_augmented.shape[1])
    spec_original = spec_original[:, :min_len]
    spec_augmented = spec_augmented[:, :min_len]
